...
*Внимание! Для корректной работы приложения должна быть создана и заполнена база данных, должно быть рабочее подключение на localhost (см. раздел [развертывание приложения](#project deployment)) и также должна быть открыта программа Library_console.py.*
